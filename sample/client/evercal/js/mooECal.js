...
var Calendar = new Class({
  ...
  options: {
    ...
    /*cEvents: [ //simulate JSON feed
      ...
      },
      {
        ...
        date:'2009-2-20T23:30:00-06:00'
      },
      {
        title:'Hair Cut',
        date:'2009-2-22T13:00:00-06:00'
      }
    ]*/
  },
  ...
  showMonth: function(){
    ...
    for (var i = 0; i < 6; i++){
      ...
      for (var j = 0; j < 7; j++) {
        ...
        var cellDate = new Date(this.calDate.getFullYear(), this.calDate.getMonth(), day);
  ...
  loadCalEvents: function(){
    ...
    for(var i = 0; i < this.options.cEvents.length; i++){
      ...
      eStart.parse(this.options.cEvents[i].date);
      eEnd.parse(this.options.cEvents[i].date);
      //time = (this.options.cEvents[i].date.toString().match(new RegExp(/(\d\d\d\d-\d\d-\d\dT)/))) ? eStart.thTime() : '';
      time = (this.options.cEvents[i].date.toString().length > 16) ? eStart.thTime() : ''; // check to see if feed event date included a time
      if((eStart >= this.viewStartDate && eStart <= this.viewEndDate) || (eEnd >= this.viewStartDate && eEnd <= this.viewEndDate)){
        while(eStart <= eEnd){
          if (eStart >= this.viewStartDate && eStart <= this.viewEndDate) {
            var title = this.options.cEvents[i].title;
            var image = this.options.cEvents[i].image;
            var url = this.options.cEvents[i].url;
            var html;
            if (image) {
              html = '<img src="'+image+'"/>';
            } else {
            ...
            var eventDiv = new Element('div', {
              'html': html
            }).store('date', eStart.getDate()).addClass('thumbnail').inject($('day' + eStart.getDate()).getChildren('div')[0]);
            if (image) {
              new Element('div', {
                'html': title
            ...
            if (this.options.view != 'month') // the month view only shows a portion of the event description
              ...
              eventDiv.store('tip:title', time).store('tip:text', this.options.cEvents[i].title);
            }
          }
          eStart.increment();
        }
